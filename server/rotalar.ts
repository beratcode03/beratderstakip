...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/tasks/:id/unarchive", async (req, res) => {
    try {
      const { id } = req.params;
      const task = await storage.updateTask(id, { archived: false, archivedAt: null });
  ...
  app.get("/api/calendar/:date", async (req, res) => {
    try {
      ...
      const activeTasks = await storage.getTasksByDate(date);
      ...
      const activeStudyHours = await storage.getStudyHours();
      const archivedStudyHours = await storage.getArchivedStudyHours();
      ...
      const activeQuestionLogs = await storage.getQuestionLogs();
      const archivedQuestionLogs = await storage.getArchivedQuestionLogs();
      ...
      const activeExamResults = await storage.getExamResults();
      const archivedExamResults = await storage.getArchivedExamResults();
  ...
  app.post("/api/question-logs", async (req, res) => {
    try {
      ...
      logActivity('Soru Kaydı Eklendi', `${validatedData.question_count} soru - ${validatedData.subject}`);
  ...
  */
  ...
  app.get("/api/download-report", async (req, res) => {
    try {
      // Fetch ALL data from storage - SİLİNMİŞ veriler de dahil (istatistikler için)
      const allTasks = await storage.getTasks();
      const archivedTasks = await storage.getArchivedTasks();
      const allTasksData = [...allTasks, ...archivedTasks];
      
      // Silinmiş kayıtları da dahil et (istatistikler için)
      const allQuestionsData = await storage.getAllQuestionLogsIncludingDeleted();
      const allExamsData = await storage.getAllExamResultsIncludingDeleted();
      const allStudyHoursData = await storage.getAllStudyHoursIncludingDeleted();
      ...
      allQuestionsData.forEach(log => {
        totalQuestions += (parseInt(log.correct_count) || 0) + (parseInt(log.wrong_count) || 0) + (parseInt(log.blank_count) || 0);
        correctAnswers += parseInt(log.correct_count) || 0;
        wrongAnswers += parseInt(log.wrong_count) || 0;
      });
      ...
      const reportData = {
        ...
        totalActivities: allTasksData.length + allQuestionsData.length + allExamsData.length + allStudyHoursData.length,
      };
  ...
  app.post("/api/send-report", async (req, res) => {
    try {
      ...
      // YALNIZCA BU AYLIK VERİLERİ GETİR - Rapor gönder tuşuna basıldığında sadece bu ayın verileri gösterilsin
      const now = new Date();
      ...
      // Silinmiş kayıtları da dahil et (istatistikler için) - SADECE BU AY
      const allQuestionLogs = (await storage.getAllQuestionLogsIncludingDeleted())
        .filter(log => log.study_date >= firstDayOfMonth && log.study_date <= lastDayOfMonth);
      const allExamResults = (await storage.getAllExamResultsIncludingDeleted())
        .filter(exam => exam.exam_date >= firstDayOfMonth && exam.exam_date <= lastDayOfMonth);
      const allStudyHours = (await storage.getAllStudyHoursIncludingDeleted())
        .filter(sh => sh.study_date >= firstDayOfMonth && sh.study_date <= lastDayOfMonth);
      
      // TÜM verileri birleştir - SADECE BU AYIN GÖREVLERİ
      const allTasks = [...tasks, ...archivedTasks]
        .filter(task => {
          const taskDate = task.dueDate || task.createdAt;
          if (!taskDate) return false;
          const dateStr = typeof taskDate === 'string' ? taskDate : new Date(taskDate).toISOString();
          return dateStr.split('T')[0] >= firstDayOfMonth && dateStr.split('T')[0] <= lastDayOfMonth;
        });
      ...
      // Calculate max TYT and AYT nets - Genel ve Branş için ayrı
      const generalExams = allExamResults.filter(e => e.exam_scope === 'full');
      const branchExams = allExamResults.filter(e => e.exam_scope === 'branch');
      
      const maxTytNet = Math.max(...generalExams.map(e => parseFloat(e.tyt_net) || 0), 0);
      const maxAytNet = Math.max(...generalExams.map(e => parseFloat(e.ayt_net) || 0), 0);
      
      // Branş denemeleri için max netler hesapla
      let maxBranchTytNet = 0;
      let maxBranchAytNet = 0;
      
      branchExams.forEach(exam => {
        if (exam.subjects_data) {
          try {
            const subjectData = JSON.parse(exam.subjects_data);
            ...
            if (exam.exam_type === 'TYT') {
              maxBranchTytNet = Math.max(maxBranchTytNet, net);
            } else if (exam.exam_type === 'AYT') {
              maxBranchAytNet = Math.max(maxBranchAytNet, net);
            }
          } catch (e) {
            console.error('Error parsing branch exam subjects_data:', e);
          }
        }
      });
      ...
      // Frequent wrong topics (minimum 1 yanlış filtresi - Eksik Olduğum Konular ile aynı)
      ...
      // Toplam çalışma saati hesapla (tüm zamandan)
      const totalStudySeconds = allStudyHours
      ...
      const totalStudySecs = totalStudySeconds % 60;
      const formattedStudyTime = `${String(totalStudyHours).padStart(2, '0')}:${String(totalStudyMinutes).padStart(2, '0')}:${String(totalStudySecs).padStart(2, '0')}`;
      ...
      const uniqueStudyDates = Array.from(new Set(allStudyHours.map((sh: any) => sh.study_date)));
      const sortedStudyDates = uniqueStudyDates.map(d => new Date(d)).sort((a, b) => a.getTime() - b.getTime());
      let longestStreak = 0;
      let currentStreak = 0;
      
      // Eğer bugün çalışma verisi varsa, en az 1 göster
      const today = new Date().toISOString().split('T')[0];
      const hasTodayActivity = uniqueStudyDates.some(date => date.startsWith(today));
      
      for (let i = 0; i < sortedStudyDates.length; i++) {
        if (i === 0) {
          currentStreak = 1;
        } else {
          const prevDate = sortedStudyDates[i - 1];
          const currDate = sortedStudyDates[i];
      ...
      // Rapor gönderildiği gün aktivite varsa en az 1 göster
      if (hasTodayActivity && longestStreak === 0) {
        longestStreak = 1;
      }
      
      // Bu ayın yanlış soru sayısı - question logs'daki wrong_topics array'lerinden hesapla
      // now, currentMonth, currentYear zaten yukarıda tanımlı
      
      // Bu ay eklenen yanlış konuların toplam sayısı (her yanlış konu 1 sayılır)
      const monthlyWrongQuestions = allQuestionLogs
        .filter(log => {
          const logDate = new Date(log.study_date);
          return logDate.getMonth() === currentMonth && logDate.getFullYear() === currentYear;
        })
        .reduce((total, log) => {
          // wrong_topics array'indeki her konu için 1 ekle
          if (log.wrong_topics && Array.isArray(log.wrong_topics)) {
            return total + log.wrong_topics.length;
          }
          // wrong_topics_json string ise parse et
          if (log.wrong_topics_json) {
            try {
              const topics = JSON.parse(log.wrong_topics_json);
      ...
      // Düzeltilen konular - yanlış konulardan checkbox ile işaretlenip çalışılanlar
      // Bu görevlerdeki "düzeltildi" veya konu adı ile eşleşen tamamlanan görevleri say
      const totalCorrectedTopics = allTasks
        .filter(task => {
          if (!task.completed || !task.completedAt) return false;
          const completedDate = new Date(task.completedAt);
          const isCurrentMonth = completedDate.getMonth() === currentMonth && completedDate.getFullYear() === currentYear;
          // Konu çalışması ile ilgili görevleri kontrol et (category bazlı veya konu adı içeren)
          const isTopicTask = task.category && task.category !== 'genel';
          return isCurrentMonth && isTopicTask;
        }).length;
      
      // Bu ay çözülen toplam soru sayısı (aylık)
      const monthlyTotalQuestions = allQuestionLogs
        .filter(log => {
          const logDate = new Date(log.study_date);
          return logDate.getMonth() === currentMonth && logDate.getFullYear() === currentYear;
        })
        .reduce((total, log) => {
          return total + (parseInt(log.correct_count) || 0) + (parseInt(log.wrong_count) || 0) + (parseInt(log.blank_count) || 0);
        }, 0);
      
      // En çok hata yapılan ve en çok doğru yapılan konular
      const topicStats: { [key: string]: { wrong: number; correct: number; subject: string } } = {};
      ...
      allQuestionLogs.forEach(log => {
        ...
        if (log.topic && log.topic.trim() !== '') {
          if (!topicStats[log.topic]) {
            topicStats[log.topic] = { wrong: 0, correct: 0, subject: log.subject };
          }
          topicStats[log.topic].wrong += wrong;
          topicStats[log.topic].correct += correct;
        }
      });
      
      const mostWrongTopicsList = Object.entries(topicStats)
        .sort((a, b) => b[1].wrong - a[1].wrong)
        .slice(0, 10)
        .map(([topic, stats]) => ({ topic, count: stats.wrong, subject: stats.subject }));
      
      const mostCorrectTopicsList = Object.entries(topicStats)
        .sort((a, b) => b[1].correct - a[1].correct)
        .slice(0, 10)
        .map(([topic, stats]) => ({ topic, count: stats.correct, subject: stats.subject }));
      
      const reportData = {
        totalQuestions: monthlyTotalQuestions, // Bu ay çözülen toplam soru sayısı
        correctAnswers,
        wrongAnswers,
        blankAnswers,
        ...
        totalActivities: allQuestionLogs.length + allExamResults.length + completedTasks.length,
        tyt_net: maxTytNet,
        ayt_net: maxAytNet,
        maxTytNet,
        maxAytNet,
        ...
        mostWrongTopicsList,  // En çok hata yapılan konular
        mostCorrectTopicsList, // En çok doğru yapılan konular
        formattedStudyTime,
        longestStudyDay: formattedLongestDay,
        longestStreak,
        monthlyWrongQuestions, // Bu ay hatalı soru etiketli tamamlanan görevler
        totalCorrectedTopics   // Deneme etiketli tamamlanan görevler
      };

      // Gmail SMTP konfigürasyonu (Basitleştirilmiş)
      const emailUser = process.env.EMAIL_USER;
      const emailPass = process.env.EMAIL_PASS;
      const emailFrom = process.env.EMAIL_FROM;

      if (!emailUser || !emailPass || !emailFrom) {
        console.error('❌ Email konfigürasyonu eksik:', {
        ...
        return res.status(400).json({
          success: false,
          message: "Email konfigürasyonu eksik. Lütfen .env dosyasında EMAIL_USER, EMAIL_PASS ve EMAIL_FROM değişkenlerini ayarlayın."
        });
      }
      ...
      try {
        ...
        console.log('✅ SMTP bağlantısı doğrulandı');
      } catch (verifyError: any) {
        console.error('❌ SMTP bağlantı hatası:', verifyError.message);
        return res.status(500).json({
          success: false,
          message: `SMTP sunucusuna bağlanılamadı: ${verifyError.message}`
        });
      }

      // Resimleri yükle - Gelişmiş yol çözümleme
      const getImagePath = (filename: string) => {
        const possiblePaths = [];
        
        // 1. Production - Electron resources (RESOURCES_PATH environment variable'dan)
        if (process.env.RESOURCES_PATH) {
          possiblePaths.push(path.join(process.env.RESOURCES_PATH, 'assets', filename));
          // Asar unpacked path de dene
          possiblePaths.push(path.join(process.env.RESOURCES_PATH, 'app.asar.unpacked', 'client', 'public', filename));
        }
        
        // 2. Development - client/public
        possiblePaths.push(path.join(process.cwd(), 'client', 'public', filename));
        
        // 3. Public folder (fallback)
        possiblePaths.push(path.join(process.cwd(), 'public', filename));
        
        // 4. Attached assets
        possiblePaths.push(path.join(process.cwd(), 'attached_assets', filename));
        
        // İlk bulunan dosyayı döndür
        for (const imagePath of possiblePaths) {
          if (fs.existsSync(imagePath)) {
            console.log('✅ Resim bulundu:', imagePath);
            return imagePath;
          }
        }
        ...
        console.error('Aranan yollar:', possiblePaths);
        throw new Error(`Resim dosyası bulunamadı: ${filename}. Lütfen .exe dosyasını yeniden kurun veya resimleri resources/assets klasörüne manuel olarak kopyalayın.`);
      };
      ...
      try {
        ...
        console.error('❌ Email resimleri yüklenemedi:', imageError.message);
        return res.status(500).json({
          success: false,
          message: `Email gönderimi için gerekli dosyalar bulunamadı: ${imageError.message}`
        });
      }
      ...
      // EMAIL_USER ve EMAIL_FROM'a gönder, her ikisi de farklıysa ikisine de git
      const recipients = emailFrom === emailUser 
        ? emailFrom 
        : `${emailUser}, ${emailFrom}`;
          ...
          <body style="margin: 0; padding: 20px; background: #F3F4F6; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            ...
            <div style="max-width: 800px; margin: 0 auto; background: linear-gradient(135deg, #E30A17 0%, #8B5CF6 50%, #E30A17 100%); padding: 25px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
              ...
              <div style="background: white; border-radius: 20px; padding: 45px; box-shadow: 0 10px 40px rgba(0,0,0,0.2);">
                ...
                <!-- Çalışılan En Uzun Gün - ÇALIŞMA SÜRESİ'nin hemen altına taşındı -->
                ${reportData.longestStudyDay ? `
                <div style="width: 100%; margin-bottom: 30px;">
                  <div style="background: linear-gradient(135deg, #DBEAFE 0%, #BFDBFE 100%); padding: 30px; border-radius: 18px; border-left: 6px solid #3B82F6;">
                ...
                <!-- YENİ İSTATİSTİKLER: En Uzun Seri, Aylık Hatalı Sorular, Düzeltilen Konular -->
                <div style="background: linear-gradient(135deg, #F3E8FF 0%, #E9D5FF 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #8B5CF6;">
                  <h3 style="color: #6B21A8; margin: 0 0 25px 0; font-size: 22px; font-weight: bold;">📊 ÖZEL İSTATİSTİKLER</h3>
                  <div style="display: table; width: 100%;">
                    <!-- En Uzun Çalışma Serisi -->
                    <div style="display: table-cell; width: 33.33%; padding-right: 10px;">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #8B5CF6;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">🔥 En Uzun Çalışma Serisi</div>
                        <div style="font-size: 42px; font-weight: bold; color: #8B5CF6;">${reportData.longestStreak || 0}</div>
                        <div style="font-size: 14px; color: #9CA3AF; margin-top: 8px;">ardışık gün</div>
                      </div>
                    </div>
                    <!-- Bu Ay Hatalı Sorular -->
                    <div style="display: table-cell; width: 33.33%; padding: 0 5px;">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #EF4444;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">❌ Bu Ay Hatalı Sorular</div>
                        <div style="font-size: 42px; font-weight: bold; color: #EF4444;">${reportData.monthlyWrongQuestions || 0}</div>
                        <div style="font-size: 14px; color: #9CA3AF; margin-top: 8px;">yanlış soru</div>
                      </div>
                    </div>
                    <!-- Düzeltilen Konular -->
                    <div style="display: table-cell; width: 33.33%; padding-left: 10px;">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #10B981;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">✅ Düzeltilen Konular</div>
                        <div style="font-size: 42px; font-weight: bold; color: #10B981;">${reportData.totalCorrectedTopics || 0}</div>
                        <div style="font-size: 14px; color: #9CA3AF; margin-top: 8px;">konu düzeltildi</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <!-- 4. Rekor Deneme Netleri - Genel Deneme -->
                ...
                <div style="background: linear-gradient(135deg, #DBEAFE 0%, #BFDBFE 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #3B82F6;">
                  <h3 style="color: #1E40AF; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">🏆 GENEL DENEME REKOR NETLERİ</h3>
                  <div style="display: table; width: 100%;">
                    ${reportData.maxTytNet > 0 ? `
                    <div style="display: table-cell; width: 50%; padding-right: 10px;">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #8B5CF6;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">TYT Rekor Net</div>
                        <div style="font-size: 42px; font-weight: bold; color: #8B5CF6;">${reportData.maxTytNet.toFixed(1)}</div>
                      </div>
                    </div>
                    ` : ''}
                    ...
                    <div style="display: table-cell; width: 50%; padding-left: ${reportData.maxTytNet > 0 ? '10px' : '0'};">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #EF4444;">
                        ...
                        <div style="font-size: 42px; font-weight: bold; color: #EF4444;">${reportData.maxAytNet.toFixed(1)}</div>
                      </div>
                    </div>
                    ` : ''}
                  </div>
                ...
                <!-- 4b. Rekor Deneme Netleri - Branş Deneme -->
                ${(reportData.maxBranchTytNet > 0 || reportData.maxBranchAytNet > 0) ? `
                <div style="background: linear-gradient(135deg, #FAE8FF 0%, #F3E8FF 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #A855F7;">
                  <h3 style="color: #7E22CE; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">📚 BRANŞ DENEME REKOR NETLERİ</h3>
                  <div style="display: table; width: 100%;">
                    ${reportData.maxBranchTytNet > 0 ? `
                    <div style="display: table-cell; width: 50%; padding-right: 10px;">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #C084FC;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">TYT Branş Rekor</div>
                        <div style="font-size: 42px; font-weight: bold; color: #C084FC;">${reportData.maxBranchTytNet.toFixed(1)}</div>
                      </div>
                    </div>
                    ` : ''}
                    ${reportData.maxBranchAytNet > 0 ? `
                    <div style="display: table-cell; width: 50%; padding-left: ${reportData.maxBranchTytNet > 0 ? '10px' : '0'};">
                      <div style="background: white; padding: 25px; border-radius: 14px; text-align: center; border: 3px solid #A855F7;">
                        <div style="font-size: 16px; color: #6B7280; margin-bottom: 10px; font-weight: 600;">AYT Branş Rekor</div>
                        <div style="font-size: 42px; font-weight: bold; color: #A855F7;">${reportData.maxBranchAytNet.toFixed(1)}</div>
                      </div>
                    </div>
                    ` : ''}
                  </div>
                ...
                <!-- 5. Sık Hata Yapılan Konular - Eksik Olduğum Konular (TÜM KONULAR) -->
                ...
                <div style="background: linear-gradient(135deg, #FEE2E2 0%, #FECACA 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #EF4444;">
                  <h3 style="color: #991B1B; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">⚠️ EKSİK OLDUĞUM KONULAR (${reportData.frequentWrongTopics.length} Konu)</h3>
                  <div style="color: #7F1D1D; font-size: 15px; line-height: 1.7;">
                    ${reportData.frequentWrongTopics.map((item, index) => `
                ...
                <!-- 9. En Çok Hata Yapılan Konu -->
                ${reportData.mostWrongTopicsList && reportData.mostWrongTopicsList.length > 0 ? `
                <div style="background: linear-gradient(135deg, #FEE2E2 0%, #FECACA 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #EF4444;">
                  <h3 style="color: #991B1B; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">❌ EN ÇOK HATA YAPILAN KONU</h3>
                  <div>
                    ${reportData.mostWrongTopicsList.slice(0, 5).map((item, index) => `
                      <div style="background: white; padding: 16px 20px; margin: 10px 0; border-radius: 10px; border-left: 4px solid #EF4444; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                          <strong style="color: #991B1B;">${index + 1}. ${item.topic}</strong>
                          <div style="font-size: 13px; color: #6B7280; margin-top: 4px;">${item.subject}</div>
                ...
                <!-- 10. En Çok Doğru Yapılan Konu -->
                ${reportData.mostCorrectTopicsList && reportData.mostCorrectTopicsList.length > 0 ? `
                <div style="background: linear-gradient(135deg, #D1FAE5 0%, #A7F3D0 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #10B981;">
                  <h3 style="color: #065F46; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">✅ EN ÇOK DOĞRU YAPILAN KONU</h3>
                  <div>
                    ${reportData.mostCorrectTopicsList.slice(0, 5).map((item, index) => `
                      <div style="background: white; padding: 16px 20px; margin: 10px 0; border-radius: 10px; border-left: 4px solid #10B981; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                          <strong style="color: #065F46;">${index + 1}. ${item.topic}</strong>
                          <div style="font-size: 13px; color: #6B7280; margin-top: 4px;">${item.subject}</div>
                ...
                <!-- En Çok Hata Yapılan Konular - Tüm Eksik Konular Listesi -->
                ${reportData.frequentWrongTopics && reportData.frequentWrongTopics.length > 0 ? `
                <div style="background: linear-gradient(135deg, #FEE2E2 0%, #FECACA 100%); padding: 30px; border-radius: 18px; margin-bottom: 30px; border-left: 6px solid #EF4444;">
                  <h3 style="color: #991B1B; margin: 0 0 20px 0; font-size: 20px; font-weight: bold;">🎯 EKSİK OLDUĞUM TÜM KONULAR (${reportData.frequentWrongTopics.length} Konu)</h3>
  ...
  app.post("/api/test-emails", async (req, res) => {
    try {
      ...
      doc.on("end", () => {
        ...
        // Test e-posta ayarları - EMAIL_USER ve EMAIL_FROM'a gönder
        const emailUser = process.env.EMAIL_USER;
        const emailFrom = process.env.EMAIL_FROM;
        const recipients = emailFrom === emailUser 
          ? emailFrom 
