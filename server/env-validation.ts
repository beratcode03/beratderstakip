...
export function validateEnvironmentVariables() {
  ...
  if (!process.env.EMAIL_USER) {
    warnings.push('⚠️  EMAIL_USER is not set. Email features will not work.');
  }

  if (!process.env.EMAIL_PASS) {
    warnings.push('⚠️  EMAIL_PASS is not set. Email features will not work.');
  }
  
  if (!process.env.EMAIL_FROM) {
    warnings.push('⚠️  EMAIL_FROM is not set. Email features will not work.');
  }
