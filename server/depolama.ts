...
export class MemStorage implements IStorage {
  ...
  private loadFromFileSync(): void {
    ...
    if (existsSync(this.dataPath)) {
      try {
        ...
        if (parsed.setupData) this.setupData = { ...parsed.setupData, createdAt: new Date(parsed.setupData.createdAt) };
  ...
  async getArchivedTasks(): Promise<Task[]> {
    ...
    return Array.from(this.tasks.values())
      .filter(task => {
        ...
        // Arşivlenme tarihini kontrol et - 1 haftadan eski olanları gösterme
        const archivedDate = task.archivedAt ? new Date(task.archivedAt).getTime() : 0;
        return archivedDate >= oneWeekAgoTime;
      })
      .sort((a, b) => {
  ...
  async getArchivedQuestionLogs(): Promise<QuestionLog[]> {
    ...
    return Array.from(this.questionLogs.values())
      .filter(log => {
        if (log.deleted || !log.archived) return false;
        
        // Arşivlenme tarihini kontrol et - 1 haftadan eski olanları gösterme
        const archivedDate = log.archivedAt ? new Date(log.archivedAt).getTime() : 0;
        return archivedDate >= oneWeekAgoTime;
      })
      .sort((a, b) => 
        new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
  ...
  async getArchivedExamResults(): Promise<ExamResult[]> {
    ...
    return Array.from(this.examResults.values())
      .filter(result => {
        if (result.deleted || !result.archived) return false;
        
        // Arşivlenme tarihini kontrol et - 1 haftadan eski olanları gösterme
        const archivedDate = result.archivedAt ? new Date(result.archivedAt).getTime() : 0;
        return archivedDate >= oneWeekAgoTime;
      })
      .sort((a, b) => 
        new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime()
  ...
  async getArchivedStudyHours(): Promise<StudyHours[]> {
    ...
    return Array.from(this.studyHours.values())
      .filter(sh => {
        if (sh.deleted || !sh.archived) return false;
        
        // Arşivlenme tarihini kontrol et - 1 haftadan eski olanları gösterme
        const archivedDate = sh.archivedAt ? new Date(sh.archivedAt).getTime() : 0;
        return archivedDate >= oneWeekAgoTime;
      })
      .sort((a, b) => 
        new Date(b.study_date).getTime() - new Date(a.study_date).getTime()
...
const sqlConnection = connectionString ? neon(connectionString) : null;
const db = sqlConnection ? drizzle(sqlConnection) : null as any;
...
export class DbStorage implements IStorage {
  ...
  async autoArchiveOldData(): Promise<void> {
    ...
    console.log(`Auto-archive completed for date: ${today}`);
  }
...
// Veritabanı bağlantısı varsa DbStorage kullan, yoksa MemStorage kullan
