...
function AdvancedChartsComponent() {
  ...
  const [completedExamErrors, setCompletedExamErrors] = useState<Set<string>>(new Set());
  const [completedQuestionErrors, setCompletedQuestionErrors] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    try {
      ...
      if (savedCompletedExamErrors) {
        setCompletedExamErrors(new Set(JSON.parse(savedCompletedExamErrors)));
      }
      if (savedCompletedQuestionErrors) {
        setCompletedQuestionErrors(new Set(JSON.parse(savedCompletedQuestionErrors)));
      }
      if (savedTytTarget) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('completedExamErrors', JSON.stringify(Array.from(completedExamErrors)));
  ...
  useEffect(() => {
    try {
      localStorage.setItem('completedQuestionErrors', JSON.stringify(Array.from(completedQuestionErrors)));
      // Aynı sekmede diğer componentleri bilgilendir
      window.dispatchEvent(new CustomEvent('localStorageUpdate'));
    } catch (error) {
  ...
  const netAnalysisData = useMemo(() => {
    return examResults.filter(exam => exam.exam_scope === 'full').map(exam => {
                        ...
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            if (checked) {
                              ...
                              // Kaynak tipine göre uygun set'i güncelle (tracking için gerekli)
                              // Ancak sadece 'question' kaynaklar "Toplam Çözülen Hata" sayacına katılır
                              if (topic.source === 'exam') {
                                setCompletedExamErrors(prev => new Set([...prev, topicKey]));
                              } else if (topic.source === 'question') {
                                setCompletedQuestionErrors(prev => new Set([...prev, topicKey]));
                              }
                              ...
                              // Uncheck durumunda da set'lerden kaldır
                              if (topic.source === 'exam') {
                        ...
                        <div className="flex items-center gap-4">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              ...
                              if (checked) {
                                if (isExamError) {
                                  setCompletedExamErrors(prev => new Set([...prev, errorTopicKey]));
                                } else {
                                  setCompletedQuestionErrors(prev => new Set([...prev, errorTopicKey]));
                                }
          ...
          {analysisMode === 'general' ? (
            ...
            branchExamData.length === 0 ? (
              ...
              <div className="space-y-6">
                ...
                {/* Branş Denemeleri Zaman Çizgisi Grafiği */}
                <div className="h-96 bg-gradient-to-br from-amber-50/30 to-orange-50/30 dark:from-amber-950/20 dark:to-orange-950/20 rounded-xl p-4">
                  <h3 className="text-lg font-bold text-center mb-4 text-amber-700 dark:text-amber-300">📈 Branş Denemesi Zaman Çizgisi</h3>
                  <ResponsiveContainer width="100%" height="85%">
                    <LineChart data={branchExamData} margin={{ top: 20, right: 60, bottom: 50, left: 40 }}>
                      <CartesianGrid strokeDasharray="4 4" stroke="currentColor" opacity={0.15} />
                      <XAxis 
                        dataKey="date" 
                        tick={{ fontSize: 12, fontWeight: 600 }}
                      ...
                      <Tooltip 
                        ...
                        labelFormatter={(label, payload) => {
                          const data = payload?.[0]?.payload;
                          return data ? `📚 ${data.examName} - ${data.subject} - ${label}` : label;
                        }}
                        formatter={(value: any, name: any) => [`${value} net`, '📊 Net Skoru']}
                      />
                      <Legend 
                        wrapperStyle={{ paddingTop: '30px', fontSize: '14px', fontWeight: 600 }}
                        iconType="line"
                      />
                      {/* Hedef netler için referans çizgileri */}
                      <ReferenceLine 
                        y={tytBranchTargetNet} 
                        stroke="#3b82f6" 
                        strokeDasharray="6 6" 
                        strokeWidth={3}
                        label={{ value: `🎯 TYT Hedef (${tytBranchTargetNet})`, position: 'right', fill: '#3b82f6', fontSize: 12, fontWeight: 600 }}
                        opacity={0.8}
                      />
                      <ReferenceLine 
                        y={aytBranchTargetNet} 
                        stroke="#a855f7" 
                        strokeDasharray="6 6" 
                        strokeWidth={3}
                        label={{ value: `🎯 AYT Hedef (${aytBranchTargetNet})`, position: 'right', fill: '#a855f7', fontSize: 12, fontWeight: 600 }}
                        opacity={0.8}
                      />
                      ...
                      <Line 
                        ...
                        stroke="#f97316" 
                        strokeWidth={5}
                        dot={{ fill: '#f97316', strokeWidth: 4, r: 8, stroke: '#ffffff' }} 
                        activeDot={{ r: 12, stroke: '#f97316', strokeWidth: 4, fill: '#ffffff' }}
                        connectNulls={true}
                        name="📚 Branş Net Skoru"
                      />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
