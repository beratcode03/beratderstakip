...
export function TodaysTasksWidget() {
  ...
  const tasks = allTasks.filter(task => !task.completed && !task.archived && !task.deleted);
    ...
    <div className="bg-card rounded-xl border border-border p-6 transition-colors duration-300 h-full flex flex-col relative overflow-hidden">
      ...
      {tasks.length === 0 ? (
        ...
        <>
          ...
          <div className={`space-y-3 flex-1 min-h-0 max-h-[500px] ${tasks.length > 3 ? 'overflow-y-auto' : 'overflow-hidden'} custom-scrollbar`}>
            {tasks.map((task) => (
              <div
                key={task.id}
