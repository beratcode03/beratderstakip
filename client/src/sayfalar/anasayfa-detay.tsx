...
export default function Homepage() {
  ...
  // Türkiye saatine göre bugünü garanti et - component mount ve periyodik kontrol
  useEffect(() => {
    const updateToday = () => {
    ...
    // Her 10 saniyede bir kontrol et (gece yarısı değişimleri için)
  ...
  const handleAutoSendEmail = async () => {
    try {
      ...
      // Rekor deneme netleri hesapla - Sadece GENEL denemeler (exam_scope === 'full')
      const generalTytExams = monthlyExamResults.filter((exam: any) => exam.exam_type === 'TYT' && exam.exam_scope === 'full');
      const generalAytExams = monthlyExamResults.filter((exam: any) => exam.exam_type === 'AYT' && exam.exam_scope === 'full');
      
      const maxTytNet = generalTytExams.length > 0 
        ? Math.max(...generalTytExams.map((exam: any) => parseFloat(exam.tyt_net || '0')))
        : 0;
      const maxAytNet = generalAytExams.length > 0
        ? Math.max(...generalAytExams.map((exam: any) => parseFloat(exam.ayt_net || '0')))
        : 0;
  ...
  const handleSendEmail = () => {
    ...
    // Send data in format expected by backend: { email, reportData }
    const emailRequest = {
      email: 'beratkaccow03@gmail.com',
      reportData: comprehensiveReportData
    };
