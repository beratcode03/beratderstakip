...
export default function Dashboard() {
  ...
  // Heatmap/takvim için TÜM verileri birleştir (arşivli + aktif)
  ...
  const generateYearlyHeatmapData = () => {
    ...
    // Tutarlı tarih formatı için toLocaleDateString kullan
    ...
    // 1 Ocak'tan bugüne kadar kaç gün var? (bugün DAHİL)
          ...
          <Card className="bg-gradient-to-br from-purple-50/50 via-card to-indigo-50/50 dark:from-purple-950/30 dark:via-card dark:to-indigo-950/30 backdrop-blur-sm border-2 border-purple-200/30 dark:border-purple-800/30 shadow-2xl">
            ...
            <CardContent className="p-4">
                      ...
                      {heatmapWeeks.map((week, weekIndex) => (
                        <div key={weekIndex} className="flex flex-col gap-1">
                          {week.map((day, dayIndex) => {
                            ...
                            // Aktivite sayısına göre renk belirleme
                            const activityCount = day.count;
                            
                            // Çalışma saati kontrolü
                            const dayStudyHoursData = allStudyHours.filter(sh => sh.study_date === day.date);
                            const totalStudyMinutes = dayStudyHoursData.reduce((total, sh) => {
                              return total + (sh.hours * 60) + sh.minutes;
                            }, 0);
                            const totalStudyHours = totalStudyMinutes / 60;
                            
                            // Mor tonlarında renk gradyan sistemi
                            let bgColor = '';
                            
                            // BUGÜN ise özel parlak mor arka plan ver
                            if (day.isToday) {
                              bgColor = 'bg-purple-600 dark:bg-purple-500 shadow-lg shadow-purple-400/50';
                            } else if (activityCount === 0) {
                              bgColor = 'bg-gray-200/80 dark:bg-gray-800/80';
                            } else if (totalStudyHours > 6) {
                              // 6+ saat yoğun çalışma - parlak mor/fuchsia vurgu
                              bgColor = 'bg-fuchsia-500 dark:bg-fuchsia-600';
                            } else if (activityCount <= 3) {
                              // Düşük aktivite - açık mor
                              bgColor = 'bg-purple-200 dark:bg-purple-800/60';
                            } else if (activityCount <= 7) {
                              // Orta aktivite - orta mor
                              bgColor = 'bg-purple-400 dark:bg-purple-700';
                            } else if (activityCount <= 12) {
                              // Yüksek aktivite - koyu mor
                              bgColor = 'bg-purple-600 dark:bg-purple-600';
                            } else {
                              // Çok yüksek aktivite - çok koyu mor
                              bgColor = 'bg-purple-800 dark:bg-purple-500';
                            }
              ...
              <div className="flex flex-col gap-3 mt-6">
                ...
                <div className="flex gap-3 items-center justify-center flex-wrap text-xs">
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 bg-gray-200 dark:bg-gray-800 rounded-md border-2 border-gray-300 dark:border-gray-700"></div>
                    <span>Yok</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 bg-purple-200 dark:bg-purple-800/60 rounded-md"></div>
                    <span>1-3</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 bg-purple-400 dark:bg-purple-700 rounded-md"></div>
                    <span>4-7</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 bg-purple-600 dark:bg-purple-600 rounded-md"></div>
                    <span>8-12</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <div className="w-4 h-4 bg-purple-800 dark:bg-purple-500 rounded-md"></div>
                    <span>13+</span>
                  </div>
                  <div className="flex items-center gap-1.5 ml-3 pl-3 border-l border-gray-300 dark:border-gray-700">
                    <div className="w-4 h-4 bg-fuchsia-500 dark:bg-fuchsia-600 rounded-md ring-2 ring-purple-400"></div>
                    <span>6+ saat çalışma</span>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              {questionLogs.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    {/* Toplam Doğru */}
                    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-50/80 via-emerald-50/60 to-green-50/80 dark:from-green-950/40 dark:via-emerald-950/30 dark:to-green-950/40 border-2 border-green-200/50 dark:border-green-700/40 p-5 hover:scale-105 transition-all duration-300 hover:shadow-xl">
                      <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-green-400/20 to-emerald-400/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          <div className="p-2.5 bg-green-100/80 dark:bg-green-900/40 rounded-xl">
                        ...
                        <div className="text-sm font-semibold text-green-700/80 dark:text-green-300/80">✓ Toplam Doğru</div>
                      </div>
                    </div>

                    {/* Toplam Yanlış */}
                    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-50/80 via-rose-50/60 to-red-50/80 dark:from-red-950/40 dark:via-rose-950/30 dark:to-red-950/40 border-2 border-red-200/50 dark:border-red-700/40 p-5 hover:scale-105 transition-all duration-300 hover:shadow-xl">
                      <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-red-400/20 to-rose-400/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          <div className="p-2.5 bg-red-100/80 dark:bg-red-900/40 rounded-xl">
                        ...
                        <div className="text-sm font-semibold text-red-700/80 dark:text-red-300/80">✗ Toplam Yanlış</div>
                      </div>
                    </div>

                    {/* Toplam Boş */}
                    <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50/80 via-yellow-50/60 to-amber-50/80 dark:from-amber-950/40 dark:via-yellow-950/30 dark:to-amber-950/40 border-2 border-amber-200/50 dark:border-amber-700/40 p-5 hover:scale-105 transition-all duration-300 hover:shadow-xl">
                      <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-amber-400/20 to-yellow-400/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          <div className="p-2.5 bg-amber-100/80 dark:bg-amber-900/40 rounded-xl">
                        ...
                        <div className="text-sm font-semibold text-amber-700/80 dark:text-amber-300/80">○ Toplam Boş</div>
                      </div>
                    </div>
                  </div>
                      ...
                      <div key={log.id} className="p-4 bg-gradient-to-r from-green-100/30 to-emerald-100/30 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border border-green-200/50 transition-all hover:scale-102 hover:shadow-md">
                        ...
                        {log.wrong_topics && log.wrong_topics.length > 0 && (
                          <div className="mt-2 text-xs text-red-600">
                            {log.wrong_topics.map((t: any) => typeof t === 'string' ? t : t.topic).join(', ')}
                          </div>
                        )}
                      </div>
                      ))}
                      ...
                      <div key={index} className="p-3 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-200 dark:border-green-800">
                        ...
                        {question.wrong_topics && question.wrong_topics.length > 0 && (
                          <div className="text-xs text-red-600 mt-1">
                            {question.wrong_topics.map((topic: any) => {
                              let topicName = '';
                              if (typeof topic === 'string') {
                                topicName = topic;
                              } else if (topic && typeof topic === 'object') {
                                topicName = topic.topic || topic.name || '';
                              }
                              if (!topicName) return '';
                              // TYT/AYT ön ekini ve "- Berat" gibi ekstra ayrıntıları kaldır
                              const cleanTopic = topicName.replace(/^(TYT|AYT)\s+[^-]+-\s*/, '').split(' - ')[0];
                              return cleanTopic;
                            }).filter(t => t).join(', ')}
                          </div>
                        )}
          ...
          <div className="space-y-4">
            ...
            <div className="min-h-[300px]">
              ...
              {archivedTab === 'studyHours' && (
                <div className={`space-y-3 ${archivedStudyHours.length > 5 ? 'max-h-[500px] overflow-y-auto pr-2' : ''}`}>
                  {archivedStudyHours.length === 0 ? (
                    <div className="text-center py-12 text-muted-foreground">
                      <Clock className="h-12 w-12 mx-auto mb-3 opacity-40" />
                      <p>Arşivlenmiş çalışma saati yok</p>
                    </div>
                  ) : (
                    archivedStudyHours.map((sh: any) => (
